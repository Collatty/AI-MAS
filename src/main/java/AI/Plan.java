...
public abstract class Plan {
	...
	public void calculatePlan() {
		...
		if (previous != null) {
		    this.plan.add(new Action(State.getInitialState().get(previous.getRow()).get(previous.getCol()),
	...
	public void calculatePlan() {
		...
		if (previous != null) {
		    if (partialPlan.size() == 0) {
			partialPlan
				.add(new Action(State.getInitialState().get(previous.getRow()).get(previous.getCol()),
					State.getInitialState().get(step.getRow()).get(step.getCol()),
