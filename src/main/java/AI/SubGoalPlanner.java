    ...
    private static boolean searchForBlock(Goal goal) {
        ...
        while (!frontier.isEmpty()) {
            ...
            if (exploringTile.hasBlock() && ((Block) exploringTile.getTileOccupant()).getType() == goal.getType()) {
                hasBlock = true;
                System.err.println("Found a correct type block for " + goal.toString() + " on tile (" + exploringTile.getCol() + "," + exploringTile.getRow() + ")");
            }
            if (exploringTile.getTileOccupant() instanceof Agent
                    && ((Agent) exploringTile.getTileOccupant()).getColor().equals(goal.getColor())) {
                hasAgent = true;
                System.err.println("Found a correct type agent for " + goal.toString() + " on tile (" + exploringTile.getCol() + "," + exploringTile.getRow() + ")");
            }
            ...
            for (Agent agent : block.getReachableAgents()) {
                ...
                if (pathToAgent.get(pathToAgent.size() - 1).getRow() == agent.getRow()
                    ...
                    for (Node node : pathToAgent.subList(1, pathToAgent.size() - 1)) {
                        if (State.getInitialState().get(node.getRow()).get(node.getCol()).isGoal() &&
                                !(State.getInitialState().get(node.getRow()).get(node.getCol()).getRow()==goal.getRow() &&
                                State.getInitialState().get(node.getRow()).get(node.getCol()).getCol()==goal.getCol() &&
                                State.getInitialState().get(node.getRow()).get(node.getCol()).getGoal().getType()==goal.getType()))
                                 {
                            State.getInitialState().get(node.getRow()).get(node.getCol()).getGoal().getPreconditions()
                                    .add(goal);
                            System.err.println("92: Adding " + goal.toString() + " as a precondition for " +
                                    State.getInitialState().get(node.getRow()).get(node.getCol()).getGoal().toString());
                        }
                    }
                }
