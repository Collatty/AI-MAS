...
public class oldAction {
    ...
    private oldAction(Type t, Dir d1, Dir d2) {
        this.actionType = t;
        this.dir1 = d1;
        this.dir2 = d2;
    }
    ...
    public static final oldAction[] EVERY;
    static {
        ArrayList<oldAction> cmds = new ArrayList<>();

        //push
        for (Dir d1 : Dir.values()) {
            for (Dir d2 : Dir.values()) {
                if (!oldAction.isOpposite(d1, d2)) {
                    cmds.add(new oldAction(Type.Push, d1, d2));
                }
            }
        }
        //pull
        for (Dir d1 : Dir.values()) {
            for (Dir d2 : Dir.values()) {
                if (d1 != d2) {
                    cmds.add(new oldAction(Type.Pull, d1, d2));
                }
            }
        }
        ...
        for (Dir d : Dir.values()) {
            cmds.add(new oldAction(d));
        }

        //Adding NoOp action
        cmds.add(new oldAction());

        // add every possible action to static final list of action objects
        EVERY = cmds.toArray(new oldAction[0]);
    }
